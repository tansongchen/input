...
在本方案中，输入不常用的单个汉字的推荐方法是以词定字，也即先想出一个包含该字的词，用冰雪四拼的编码打出该词后，从词里截取相应的单字。以词定字的快捷键为 `[` 和 `]`，按 `[` 上屏词的首字，按 `]` 上屏词的末字。

例如，想打「熙」这个单字时，想到它出现在四字词「熙熙攘攘」中，因此先输入 `xxrr`，此时「熙熙攘攘」出现在首选：

![rime](https://images.tansongchen.com/1738504802.png)

然后按 `[` 即可上屏「熙」。

使用以词定字要求用于定字的词必须处于选中状态。一般来说，可以通过追加编码的方式把词提升到首选来选中该词，但如果打完全部编码后词仍然不在首选，就需要先用光标键选中该词再定字。例如，想用「采薇」一词来输入「薇」，但打完词的全部编码 csuiai 之后词仍然排在第三位，此时就需要先用右箭头（或下箭头）选中该词，再按 `]` 完成定字。

![rime](https://images.tansongchen.com/1738505168.png)
...
当忘记某个音节的拼写，或者知道其中一个读音想查询其他的读音时，可以使用拼音反查功能。拼音反查需要用 `a` 键引导。例如，输入 `azhe`，可以查询到「着」的 6 个读音对应的拼写：

![rime](https://images.tansongchen.com/1739419841.png)

### 笔画反查

笔画反查依赖于 Rime 自带的「五笔画 / stroke」方案。笔画反查需要用 `u` 引导，并用 `e`, `i`, `u`, `o`, `a` 来输入横竖撇点折。例如，输入 `ueiuoa` 可以查询到「札」的读音：

![rime](https://images.tansongchen.com/1739419929.png)

## 重复上屏

若已经输入过一些内容，可以用 `i` 键来调出上屏历史，然后选择上屏。例如，使用 `yfu` 输入「重复」之后，「重复」就出现在 `i` 的首选：

![rime](https://images.tansongchen.com/1739419533.png)

## 符号

`i` 键同时还承担了输入特殊符号的功能。例如，输入 `ia` 可以得到 a 的各种变体字母：

![rime](https://images.tansongchen.com/1739419556.png)

具体的符号内容和 Rime 自带的 `symbols.yaml` 相同，只是引导键变成了 `i`。
...
笔画辅助码是在单音节词的音节码输入完成之后直接继续输入笔画来筛选的办法，其中横竖撇点折分别用 `e`, `i`, `u`, `o`, `a` 来表示，这与反查所使用的笔画码相同。笔画最多可以输入 4 笔。例如，输入生僻字「芫」，在音节码 `kooe` 输入完成之后继续加两个笔画 ei，即出现在首页。
